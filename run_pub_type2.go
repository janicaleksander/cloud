...
func main() {
	...
	// Start Type2 publisher with random delay (1-5 seconds)
	startPublisher(publisher, func() any { return event.NewType2Event() }, utils.Delay(time.Second*time.Duration(rand.Intn(randomDelayRange)+1)))

	slog.Info("Type2 publisher started (random 1-5s delay)")
